...
exports.commands = {
	...
	emoticon: {
		add: (target, room, user) => {
			...
			for (const u in targetSplit) targetSplit[u] = targetSplit[u].trim();
		...
		list: (target, room, user) => {
			...
			for (const emote in emoticons) reply += `(${emote} <img src="${emoticons[emote]}" height="${size}" width="${size}">)`;
	...
	randemote: () => {
		...
		const e = Object.keys(emoticons)[Math.floor(Math.random() * Object.keys(emoticons).length)];
		this.parse(e);
	},
