...
class ExpFunctions {
	...
	grantExp() {
		Users.users.forEach(user => {
			if (!user || !user.named || !user.connected || !user.lastChatMessage) return;
			if (Date.now() - user.lastChatMessage > 300000) return;
			...
			EXP.writeExp(user.id, amount, newTotal => {
				...
				if (oldLevel < level) {
					...
					case 15:
						...
						break;
					case 20:
						Economy.logTransaction(`${user.name} received a Profile Background for reaching level ${level}.`);
						Monitor.log(`${user.userid} has earned a Profile Background for reaching level ${level}!`);
						if (!user.tokens) user.tokens = {};
						user.tokens.bg = true;
						reward = `a Profile Background. To claim your profile background, use the command /usetoken bg, [img].`;
						break;
					case 25:
						Economy.logTransaction(`${user.name} received a Profile Music for reaching level ${level}.`);
						Monitor.log(`${user.userid} has earned a Profile Music for reaching level ${level}!`);
						if (!user.tokens) user.tokens = {};
						user.tokens.music = true;
						reward = `a Profile Music. To claim your profile music, use the command /usetoken music, [song url], [title of the song].`;
						break;
					case 30:
	...
	exp(target, room, user) {
		...
		if (target || !target && this.broadcasting) {
			...
			EXP.readExp(user.id, exp => {
				...
				expData += `Level 5 unlocks a free Custom Avatar. <br /><br />`;
				expData += `Level 10 unlocks a free Profile Title. <br /><br />`;
				expData += `Level 15 unlocks a free Emote. <br /><br />`;
				expData += `Level 20 unlocks a free Profile Background. <br /><br />`;
				expData += `Level 25 unlocks a free Profile Song. <br /><br />`;
				expData += `Level 30 unlocks a free Custom Color.  <br /><br />`;
				expData += `Level 35 unlocks a free Chatroom. <br /><br />`;
				expData += `Level 40 unlocks a free Room Shop. <br /><br />`;
				expData += `Level 45 unlocks a free Userlist Icon.<br /><br />`;
