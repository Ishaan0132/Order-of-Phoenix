...
exports.Server = {
	nameColor: (name, bold, userGroup) => {
		const userGroupSymbol = `${Users.globalAuth.get(toID(name)) ? `<strong><font color=#948A88>${Users.globalAuth.get(toID(name)).substr(0, 1)}</font></strong>` : ``}`;
	...
	messageSeniorStaff: (message, pmName, from) => {
	...
	regdate: (target, callback) => {
		...
		const req = https.get('https://pokemonshowdown.com/users/' + target + '.json', res => {
			let data = '';
			res.on('data', chunk => {
				data += chunk;
			}).on('end', () => {
	...
	reloadCSS: () => {
		...
		const req = https.get('https://play.pokemonshowdown.com/customcss.php?server=' + (Config.serverid || cssPath), () => {});
		req.end();
	},
